...
private HospitalServiceImple hospitalService = new HospitalServiceImple();
	...
	public Response post(@FormParam("hospital_id") int hospital_id,
		...
		return state ? Response.ok().entity(hospital).build() : Response.status(Status.BAD_REQUEST).build();
	}
	...
	public Response put(@FormParam("hospital_id") int hospital_id,
		...
		return this.hospitalService.updateHospital(hospital) ? Response.ok().entity(hospital).build() : Response.status(Status.BAD_REQUEST).build();
	}
	
	@DELETE
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	@Produces(MediaType.APPLICATION_JSON)
	public Response delete(@FormParam("hospital_id") int hospital_id) {
		return this.hospitalService.deleteHospital(hospital_id) ? Response.status(Status.CREATED).build() : Response.status(Status.BAD_REQUEST).build();
	}
	
	@GET
	@Path("/{id}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response findById(@javax.ws.rs.PathParam("id") int id) {
		return Response.ok().entity(this.hospitalService.findById(id)).build();
	}
	
	@GET
	@Path("room/{id}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response findByRoomId(@javax.ws.rs.PathParam("id") String id) {
		return Response.ok().entity(this.hospitalService.findByRoomId(id)).build();
	}
}
